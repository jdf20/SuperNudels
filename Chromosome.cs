...
namespace SMS
{
    class Chromosome
    {
        ...
        public Chromosome(Chromosome HomChrom1, Chromosome HomChrom2, Organism parent)
        {
            ...
            int WhitegRNAlevel = parent.GetTransgeneLevel("transgene_White_gRNA");
            int NudelgRNAlevel = parent.GetTransgeneLevel("transgene_Nudel_gRNA");
            ...
            else
            {
                ...
                if (Cas9level > 0)
                {
                    for (var i = 0; i < HC1.GeneLocusList.Count; i++)
                    {
                        ...
                        {
                            if (HC1.GeneLocusList[i].GeneName == "Nudel" && NudelgRNAlevel > 0)
                            {
                                if (HC1.GeneLocusList[i].AlleleName == "WT")
                                {
                                    if (Cas9level >= Simulation.random.Next(0, 101))
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                if (Cons >= (float)Simulation.random.NextDouble())
                                                    HC2.GeneLocusList[i].AlleleName = "R1(Non-Silent)";
                                                else
                                                    HC2.GeneLocusList[i].AlleleName = "R1(Silent)";
                                            }*/
                            ...
                            if (HC1.GeneLocusList[i].GeneName == "White" && WhitegRNAlevel > 0)
                            {
                                if (HC1.GeneLocusList[i].AlleleName == "WT")
                                {
                                    if (Cas9level >= Simulation.random.Next(0, 101))
                    ...
                    for (var i = 0; i < HC2.GeneLocusList.Count; i++)
                    {
                        if (HC1.GeneLocusList[i].GeneName == HC2.GeneLocusList[i].GeneName)
                        {
                            if (HC2.GeneLocusList[i].GeneName == "Nudel" && NudelgRNAlevel > 0)
                            {
                                if (HC2.GeneLocusList[i].AlleleName == "WT")
                                {
                                    if (Cas9level >= Simulation.random.Next(0, 101))
                            ...
                            if (HC2.GeneLocusList[i].GeneName == "White" && WhitegRNAlevel > 0) 
                            {
                                if (HC2.GeneLocusList[i].AlleleName == "WT")
                                {
                                    if (Cas9level >= Simulation.random.Next(0, 101))
